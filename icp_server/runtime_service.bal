...
service /icp on httpListener {
    ...
    resource function post heartbeat(@http:Payload json heartbeatJson) returns types:HeartbeatResponse|error? {
        do {
    ...
    resource function post deltaHeartbeat(types:DeltaHeartbeat deltaHeartbeat) returns types:HeartbeatResponse|error? {
        do {
